# Hi, I'm Jiyaro Joseph 👋

Frontend Developer focused on **UI reliability, usability systems, and interaction correctness**.  

---

## 💫 About Me:
- 🛠 Focused on fixing **UI interaction bugs, event handling issues, and usability flaws**
- 🎯 Goal: Build **robust, predictable frontend systems**
- 🧠 Interested in **testing, UI correctness, and real-world edge case handling**

---

## 🚀 Key Contributions

### 🟢 RUXAILAB

* **PR #2010** — Fixed *status filter dropdown interaction bug* on Studies page
  → Resolved click handling issue where direct label clicks failed to open dropdown (UI event handling flaw)
  https://github.com/uramakilab/remote-usability-lab/pull/2010

* **PR #1997** — Fixed *incorrect redirection in issue templates*
  → Corrected broken links in `CONTRIBUTING.md`, ensuring proper navigation for bug/feature reporting
  https://github.com/uramakilab/remote-usability-lab/pull/1997

* **PR #1984 (closed)** — Investigated *hyperlink popup overflow in editor*
  → Identified layout constraint issue and proposed fix for UI boundary handling
  https://github.com/uramakilab/remote-usability-lab/pull/1984

---

### 🟡 FOSSASIA

* **PR #918** — Fixed *broken CI badge*
  → Replaced deprecated Travis CI with GitHub Actions for correct build status display
https://github.com/fossasia/fossasia.org/pull/918 
---


---

## 🧠 Technical Strengths
- Frontend Debugging (DOM events, state issues, UI edge cases)
- Vue.js fundamentals (components, reactivity, event handling)
- Writing clean, maintainable fixes
- Understanding usability + UX behavior

---

## 🛠 Tech Stack
- Languages: JavaScript, Python  
- Frontend: React.js, Next.js, Vue.js, HTML, CSS, Tailwind
- Tools: Git, GitHub, VS Code  

---

## 📊 GitHub Stats:
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=JiyaroJoseph&show_icons=true&theme=tokyonight)
![GitHub Streak](https://streak-stats.demolab.com?user=JiyaroJoseph&theme=tokyonight)

---

## 📌 Featured Work
- Spark — Social impact collaboration platform  
- Vyapaar Connect — Business networking project  

---

## 📈 Current Goal
To contribute to production-level open source systems and deliver reliable frontend solutions.

---
